...
Policy generators define ACM policies and must be updated with version-specific names and selectors. Each reference has different policy generator files to update. PolicyGenerator and policy names must align to the OpenShift version number.

**What to change**:
```yaml
# PolicyGenerator metadata name
metadata:
  name: core-baseline-22  # → core-baseline-23
  # OR
...
policies:
  - name: core-cluster-config-4.22  # → core-cluster-config-4.23
...
- Policy generator policy names must use version numbers (e.g., `core-cluster-config-4.23`)
...
- Descriptive text in metadata.yaml, eg `This reference was designd for OpenShift 4.22`
...
1. Verify no stale version references remain:
   ```bash
   grep -r "4\.22" --include="*.yaml" telco-* | grep -v "stable-4.21" | grep -v "README"
   ```
