...
2. Create a new directory `openshift`, copy the [install-config.yaml](install-config.yaml) and the [agent-config.yaml](agent-config.yaml) files and modify them to fit your environment.
...
3. Generate the ABI ISO image with the following command:
   ...
   `openshift-install --dir openshift agent create image`
4. Load the generated ISO into each node's local disk and proceed with the OpenShift installation. To monitor the process use:
   ...
   `openshift-install --dir openshift agent wait-for bootstrap-complete --log-level=info`
   `openshift-install --dir openshift agent wait-for install-complete`
