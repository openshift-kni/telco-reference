...
This directory contains the key components of the TNC Virtualized Enabled Workload
(VEWL) configuration:
...
The repository includes several PolicyGenerator CRs named `vewl-xxx.yaml` and `core-xxx.yaml` at
...
* `core-baseline` contains fixed required RDS content such as operator hub,
...
* `core-overlay` contains RDS content where updates and patches are expected.
...
* `vewl-odf-internal` configures the OpenShift cluster to use ODF in internal
  ...
  configuration.
* `core-customer-policies` contains policies which release/un-pause custom
...
* `telco-core-upgrade-19` contains policies to prepare a cluster for an OpenShift
  version upgrade (catalog source update, MCP maxUnavailable adjustment).
* `core-upgrade-finish`and `core-upgrade-workers-2` both contain post-upgrade policies to restore MCP
