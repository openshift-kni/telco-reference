...
The repository includes several PolicyGenerator CRs named "core-xxx.yaml" and `rhoso-xxx.yaml` at this
...
* `rhoso-additions` contains configuration added by TNC
* `rhoso-odf-internal` configures the OpenShift cluster to use ODF in internal mode
* `rhoso-odf-external` configures the OpenShift cluster to use ODF in external mode
* `rhoso-kafka` configures OpenShift logs and metrics forwarding to kafka
* `rhoso-auth-idm.yaml` contains policies to configure OAUTH to use LDAP for authentication
...
- rhoso-ceph
  ...
  - `ceph-conf` - Content from `/etc/ceph/ceph.conf` of the Ceph cluster
- rhoso-keys
  - `ansible-private` - Private SSH key for ansible
  - `ansible-public` - Public SSH key for ansible
  - `migration-private` - Private SSH key for NOVA migration
  - `migration-public` - Public SSH key for NOVA migration
- osp-secret
