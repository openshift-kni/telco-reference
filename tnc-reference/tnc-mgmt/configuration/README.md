...
The full TNC Management cluster configuration can be applied using an ArgoCD application pointing to the kustomization.yaml in this directory.
...
ArgoCD is one of the main key components of the management cluster, because is in charge of managing deployment and configuration of the infrastructure managed by the management cluster, using a GitOps methodology. But, at the same time, we can deploy the management cluster using ArgoCD (recommended procedure). Therefore, to have a management cluster with ArgoCD, first, we have to have ArgoCD to create the management cluster. This is the init phase, and it is optional if you already fulfilled this requirement.
...
The ArgoCD application will be created on your cluster and will start installing and configuring all the needed management cluster components. Note that at this point the ArgoCD application is also being managed via gitops and any changes to the application should be done in git as well.
