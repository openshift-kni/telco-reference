...
The repository includes several PolicyGenerator CRs named "mgmt-xxx.yaml" at this
...
* `mgmt-baseline` contains baseline configuration for TNC MGMT cluster
* `mgmt-odf-internal` configures the OpenShift cluster to use ODF in internal mode
* `mgmt-odf-external` configures the OpenShift cluster to use ODF in external mode
* `mgmt-kafka` configures OpenShift logs and metrics forwarding to kafka
* `mgmt-auth-idm.yaml` contains policies to configure OAUTH to use LDAP for authentication
* `mgmt-finish` contains policies which release/un-pause MachineConfigPool worker
...
- mgmt: "true"           - To apply the TNC MGMT policies
