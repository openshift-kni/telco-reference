...
- The name of the directory corresponds to the rendered filename, such that a
  ...
  `../../install/extra-manifests/03-sctp-machine-config-{master|worker}.yaml`
- The directory must contain a `build.sh` which is run with $MCPROLE set to the
  ...
  `../../install/extra-manifests/*.yaml` and to do an integrity check as part of
...
- Run `make` to render the `../../install/extra-manifests/*.yaml`
