...
## Convert Butane to ignition and use it with ClusterInstance's `ignitionConfigOverride` (recommended)
...
2. Copy the output from and place it in ClusterInstance's `.spec.nodes[index].ignitionConfigOverride` (same level as `.rootDeviceHints`)
...
## Convert Butane to MachineConfig and use it with ClusterInstance's `extraManifestsRefs` (optional)

Butane binary can also generate a MachineConfig CR, which can be included in the extra-manifest configMap and passed in via `.spec.extraManifestsRefs`
