  ...
  sourceFiles:
    - fileName: PerformanceProfile.yaml
      ...
      metadata:
        name: openshift-node-performance-profile
      spec:
        # Use the additionalKernelArgs list as defined in telco-ran/configuration/reference-crs/node-tuning-operator/{x86_64|aarch64}/PerformanceProfile.yaml
...
If a cpu governor is configured via the additional kernel arguments, and the configuration is managed through Policy, this must be removed according to the `complianceType` of the policy. If the `complianceType` is configured to `mustonlyhave`, then this can be accomplished by setting the `additionalKernelArgs` to the original list as defined in the `telco-ran/configuration/reference-crs/node-tuning-operator/{x86_64|aarch64}/PerformanceProfile.yaml` file. This is shown below:
